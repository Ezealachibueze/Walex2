...
public class Brocade {
	...
	public List<StreamingCont>  listBrocade() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats='brocade'");
	...
	public  List <StreamingCont>list() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats like '%brocade%'  order by id desc limit 8");
			ResultSet rs=pst.executeQuery();
