...
public class Velvetlaces {

	DatabaseCon db=new DatabaseCon();
	
	public List<StreamingCont>  listVelvet() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats='velvet'");
	...
	public  List <StreamingCont>list() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats like '%velvet%'  order by id desc limit 8");
			ResultSet rs=pst.executeQuery();
