...
public class StreamingCont   extends HttpServlet implements Serializable{
	...
	private String id;
	private String name;
	private String price,quatity,totalMoney,addUp,itemComment;
	...
	private static final long serialVersionUID = 1L;
	 ...
	 private List<StreamingCont>  list,list1,list2,listSeq,listVev,
	 listSilk,listorganza,listbrocade;
	 
	 private List<StreamingCont> seqList,velList,organList,silkList,brocadeList;
	...
	public StreamedContent getStream() throws FileNotFoundException {
		...
		if(face.getCurrentPhaseId()==PhaseId.RENDER_RESPONSE) {
			...
			int women =Integer.parseInt(face.getExternalContext().getRequestParameterMap().get("ew"));
				
			bbb=sc.newImage(women);
	...
	public StreamedContent getStream1() {
		...
		if(face.getCurrentPhaseId()==PhaseId.RENDER_RESPONSE) {
			...
			int women =Integer.parseInt(face.getExternalContext().getRequestParameterMap().get("c"));
	...
	public String delete() {
		...
		FacesContext context=FacesContext.getCurrentInstance();
		 int a=Integer.parseInt(context.getExternalContext().getRequestParameterMap().get("del"));
             sc.drop(a);  
     		new RetrieveConnection().totalAmount(new StreamingCont().http());
             return null;
			}
...
//	     System.out.println(x3);
	...
	public StreamedContent getMean() {
		...
		if(face.getCurrentPhaseId()==PhaseId.RENDER_RESPONSE) {
			...
			int women =Integer.parseInt(face.getExternalContext().getRequestParameterMap().get("dddd"));
				
			bbb= new Check().dir(women);
	...
	public StreamedContent getStreaming() {
		...
		if(face.getCurrentPhaseId()==PhaseId.RENDER_RESPONSE) {
			...
			int women =Integer.parseInt(face.getExternalContext().getRequestParameterMap().get("sequin"));
