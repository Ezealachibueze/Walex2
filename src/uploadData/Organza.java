...
public class Organza {
	...
	public List<StreamingCont>  listOrganza() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats='organza'");
	...
	public  List <StreamingCont>list() {
		...
		try {
			...
			PreparedStatement pst=con.prepareStatement("select * from walex where cats like '%organza%'  order by id desc limit 8");
			ResultSet rs=pst.executeQuery();
